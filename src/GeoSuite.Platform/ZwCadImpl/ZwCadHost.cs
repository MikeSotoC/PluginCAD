...
namespace GeoSuite.Platform.ZwCadImpl;

public class ZwCadHost : ICadHost
{
    ...
    private void EnsureLayerExists(string name, Transaction tr, string colorIndex = "7")
    {
        ...
        if (!lt.Has(name))
        {
            ...
            var color = short.TryParse(colorIndex, out var c) ? ZwSoft.ZwCAD.Colors.Color.FromColorIndex(ZwSoft.ZwCAD.Colors.ColorMethod.ByAci, c) : ZwSoft.ZwCAD.Colors.Color.White;
            ltr.Color = color;
            lt.Add(ltr);
